    ...
    public class Store : Entity
    {
        ...
        protected override void FillData(DataTable dt)
        {
            if (dt.Rows.Count == 1)
            {
                ...
                ResidenceId = dt.Rows[0]["КодМестаХранения"] == DBNull.Value
                    ? null
                    : (int?) dt.Rows[0]["КодМестаХранения"];
                ...
                ManagerId = dt.Rows[0]["КодРаспорядителя"] == DBNull.Value
                    ? null
                    : (int?) dt.Rows[0]["КодРаспорядителя"];
                ManagerSubdivisionCode = dt.Rows[0]["КодПодразделенияРаспорядителя"] == DBNull.Value
                    ? null
                    : (int?) dt.Rows[0]["КодПодразделенияРаспорядителя"];
            ...
            for (var i = 0; i < dt.Rows.Count; i++)
                storeList.Add(
                    new Store
                    {
                        ...
                        ResidenceId = dt.Rows[i]["КодМестаХранения"] == DBNull.Value
                            ? null
                            : (int?) dt.Rows[i]["КодМестаХранения"],
                        ResourceId = Convert.ToInt32(dt.Rows[i]["КодРесурса"]),
                        KeeperId =
                            dt.Rows[i]["КодХранителя"] == DBNull.Value ? null : (int?) dt.Rows[i]["КодХранителя"],
                        ManagerId = dt.Rows[i]["КодРаспорядителя"] == DBNull.Value
                            ? null
                            : (int?) dt.Rows[i]["КодРаспорядителя"],
                        ManagerSubdivisionCode = dt.Rows[i]["КодПодразделенияРаспорядителя"] == DBNull.Value
                            ? null
                            : (int?) dt.Rows[i]["КодПодразделенияРаспорядителя"],
                        AgreementCode = dt.Rows[i]["КодДоговора"] == DBNull.Value
                            ? null
                            : (int?) dt.Rows[i]["КодДоговора"],
        ...
        public bool IsValidAt(DateTime d)
        {
            ...
            var dt = DBManager.GetData(SQLQueries.SELECT_TEST_ЛицаСкладаДействуют, CN, CommandType.Text, sqlParams);
        ...
        public int Save(bool isNew = false)
        {
            ...
            sqlParams.Add("@КодМестаХранения",
                StoreTypeId >= 20 ? ResidenceId == 0 ? DBNull.Value : (object) ResidenceId : DBNull.Value);
            ...
            sqlParams.Add("@КодПодразделенияРаспорядителя",
                ManagerSubdivisionCode == 0 ? DBNull.Value : (object) ManagerSubdivisionCode);
            ...
            sqlParams.Add("@По",
                To.HasValue ? (object) (To > DateTime.MinValue ? To.Value.AddDays(-1) : To) : DBNull.Value);

            var outputParams = new Dictionary<string, object>();
            DBManager.ExecuteNonQuery(SQLQueries.SP_Лица_InsUpd_Склады, CommandType.StoredProcedure, Config.DS_person,
        ...
        public static string GetOstatokResource(string _Sklad, string _Resource, string _Unit, DateTime _DateTo,
            bool IncludeSubRes, string _VidOstatkov)
        {
            if (string.IsNullOrEmpty(_Sklad)) return "";
            var rez = "";
            var cm = new SqlCommand("sp_ОстатокНаСкладе");
            ...
            var vid_ost = cm.Parameters.Add("@ВидОстатка", SqlDbType.Int);
            ...
            var ost = cm.Parameters.Add("@Остаток", SqlDbType.Float);
            ...
            try
            {
                cm.Connection.Open();
                cm.ExecuteNonQuery();
                rez = ConvertExtention.Convert.Object2Str(ost.Value);
            }
            catch (Exception ex)
            {
        ...
        ///     ТипСклада
        /// </summary>
        public StoreType StoreType => StoreTypeId > 0 ? new StoreType(StoreTypeId.ToString()) : null;

        /// <summary>
        ///     МестоХранения
        /// </summary>
        public Residence Residence =>
            ResidenceId != null && ResidenceId > 0 ? new Residence(ResidenceId.ToString()) : null;
        ...
        public Resource Resource
        {
            get
            {
                ...
                else if (_resource == null || _resource.Id != ResourceId.ToString())
                    _resource = new Resource(ResourceId.ToString());
                return _resource;
            }
        }
        ...
        ///     Хранитель
        /// </summary>
        public Person Keeper => KeeperId != null && KeeperId > 0 ? new Person(KeeperId.ToString()) : null;

        /// <summary>
        ///     Распорядитель
        /// </summary>
        public Person Manager => ManagerId != null && ManagerId > 0 ? new Person(ManagerId.ToString()) : null;
