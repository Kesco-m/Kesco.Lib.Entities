...
FROM	Розетки WHERE КодРасположения = @Кодрасположения
...
WHERE NOT EXISTS(SELECT NULL FROM fn_ВсеОснования(КодДокумента))";
        ...
        public static string SELECT_СвязиДокументовВытекающиеИзОснованийПоТипу(string id, string fieldId, string typeList)
        {
            return
                string.Format(
                    @"{0} INNER JOIN vwСвязиДокументов (nolock) ON КодДокумента = vwСвязиДокументов.КодДокументаВытекающего 
                    WHERE vwСвязиДокументов.КодДокументаОснования = {1} 
                    AND vwСвязиДокументов.КодПоляДокумента = {2} 
                    AND КодТипаДокумента IN ({3})
...
CREATE TABLE #ДвиженияНаСкладах (КодДвиженияНаСкладе int PRIMARY KEY, КодОтправкиВагона int, ОтправкаВагона varchar(150), 
				...
				Количество float, КодЕдиницыИзмерения int,ЕдиницаРус nvarchar(10), Точность int,
				ЦенаБезНДС money, КодСтавкиНДС int, Величина100 float, 
				СуммаБезНДС money, СуммаНДС money, Всего money,
				КодСтраныПроисхождения int, СтранаПроисхождения varchar(80),
				КодТаможеннойДекларации int, ТаможеннаяДекларация varchar(300), Порядок int, Изменил int, Изменено datetime)
...
UPDATE #ДвиженияНаСкладах SET Точность = 0 WHERE Точность IS NULL
