...
";
        ...
        /// <summary>
        ///     Строка запроса: Получение информации из vwADSI_AllUsers по коду сотрудника
        /// </summary>
        public const string SELECT_ADSI_ПоКодуСотрудника = @"
--Получение информации из vwADSI_AllUsers по коду сотрудника и его GUID
SELECT  * 
...
FROM	(SELECT	Сотрудники.КодЛица КодЛицаСотрудника, vwДолжности.КодЛица КодЛицаКомпанииСотрудника, vwДолжности.Должность ДолжностьСотрудника,
        ...
        /// <summary>
        ///     Строка запроса: Получение дополнительных параметров, выполненнных по указанию, доступных сотруднику
        /// </summary>
        public const string SP_ДопПараметрыУказанийИТ = @"
--Получение информации о дополнителных правах сотрудника
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_ДопПраваСотрудника]') AND type in (N'P', N'PC'))
    EXEC sp_ДопПраваСотрудника @КодСотрудника
ELSE
    SELECT 0 КодДопПараметраУказанийИТ, NULL ЕстьПраво WHERE 1 = 0";


        /// <summary>
        ///     Получение списка дополнительных прав
        /// </summary>
        public const string SELECT_AdvancedGrants = "SELECT * FROM ДопПараметрыУказанийИТ ORDER BY ПорядокВывода";
...
@Фамилия nvarchar(50), @Имя nvarchar(50), @Отчество nvarchar(50), @lang char(2)
...
SET @ЕстьРеквизиты = 1
SELECT @Фамилия = ФамилияРус, @Имя = ИмяРус, @Отчество = ОтчествоРус 
...
@Фамилия Фамилия, @Имя Имя, @Отчество Отчество, @ЕстьРеквизиты ЕстьРеквизиты
WHERE @КодЛица = @КодЛица
       ";
        ...
        ///     Unlock сотрудника
        /// </summary>
        public const string Сотрудник_Unlock = @"
            exec sp_UserUnlock @КодСотрудника";
...
WHERE   ((@Потомки = 1 AND ((@Код = 0 AND r.Parent IS NULL) OR (@Код <> 0 AND r.Parent = @Код)))
        OR (@Потомки = 0 AND r.КодРасположения = @Код))
        OR (@Потомки = 2 AND (r.КодРасположения IN ({0}) OR r.Parent IN ({0}))) 
        AND r.Закрыто=0                        
ORDER BY r.L
";
...
WHERE Parent.КодРасположения IN ({0}) AND Child.Закрыто=0 AND NOT EXISTS(SELECT * FROM #Расположения X WHERE Child.КодРасположения = X.КодРасположения)
ORDER BY Parent.L
...
WHERE   Расположение LIKE '{0}' 
    AND ('{1}' = '-1' OR EXISTS (SELECT 1 FROM vwРасположения AS t2 WHERE t2.КодРасположения IN ({1}) AND t1.L >= t2.L AND t1.R <= t2.R))  
    AND Закрыто=0
ORDER BY L
...
SELECT  [КодРасположения],
        ...
        4 BitMask
FROM    vwРасположения 
WHERE   (('{1}' = '-1' AND Parent IS NULL) OR КодРасположения IN ({1})) 
        AND NOT EXISTS(SELECT * FROM #Расположения X WHERE vwРасположения.КодРасположения = X.КодРасположения) AND Закрыто=0
...
SELECT КодУслуги,НомерАбонента,ФИОАбонента,Email,СтатусАбонента,ДатаПоследнегоИзмененияСтатуса,ПричинаПоследнегоИзменения,ОписаниеУслугиТарифногоПлана,ДатаНачалаДействия
,ДатаОкончанияДействия,КодУслугиТарифногоПлана,ДатаАктивацииАбонента,СтоимостьВключения,СтоимостьВдень,СтоимостьВмесяц,ГигабайтВмесяц,КодДоговора 
FROM ИмпортБилайн (nolock)";

        /// <summary>
        ///     Строка запроса: Поиск услуг провайдера по коду
        /// </summary>
        public static string SELECT_УслугиПровайдераПоID =
            string.Format(@"{0} WHERE КодУслуги = @id", string.Format(SELECT_УслугиПровайдера, ""));

        /// <summary>
        ///     Удалить услугу
        /// </summary>
        public const string DELETE_ID_УслугиПровайдера = @"
DELETE FROM Инвентаризация.dbo.ИмпортБилайн where КодУслуги = @id
";
...
UPDATE Инвентаризация.dbo.ИмпортБилайн SET 
    СтоимостьВключения = @СтоимостьВключения, 
    СтоимостьВДень = @СтоимостьВДень, 
    СтоимостьВмесяц = @СтоимостьВМесяц, 
    ГигабайтВМесяц = @ГигабайтВМесяц
WHERE КодУслуги = @id
...
SELECT КодУслугиТарифногоПлана,ОписаниеУслугиТарифногоПлана, ОписаниеУслугиТарифногоПлана, NULL, СтоимостьВключения, СтоимостьВдень, СтоимостьВмесяц, ГигабайтВмесяц FROM ИмпортБилайн T WHERE КодУслуги = @id
...
SELECT	НомерАбонента, КодУслугиТарифногоПлана, ДатаНачалаДействия, ДатаОкончанияДействия, X.СтоимостьВключения, X.СтоимостьВдень, X.СтоимостьВмесяц, GETUTCDATE()
FROM	ИмпортБилайн T INNER JOIN
		ТипыДопУслугСотОператора X ON T.КодУслугиТарифногоПлана = X.КодТипаДопУслугиСотОператора	
WHERE	T.СтоимостьВмесяц IS NOT NULL AND
...
FROM	ИмпортБилайн T INNER JOIN 
		vwSIMКартыДопУслугиСотОператора SIM ON	SIM.НомерТелефона = T.НомерАбонента 
												...
												AND SIM.СтоимостьВмесяц = T.СтоимостьВмесяц
...
--Получение информации о должнотях сотрудника
...
Мультимедиа, Net, НазваниеNet, Драйвер, Ресурс) 
...
UPDATE МоделиОборудования SET КодРесурса=@КодРесурса, КодТипаОборудования=@КодТипаОборудования,
CPU=@CPU, РазмерМонитора=@РазмерМонитора, КодТипаПамяти=@КодТипаПамяти, БанковПамяти=@БанковПамяти, Мультимедиа=@Мультимедиа,
Net=@Net, НазваниеNet=@НазваниеNet, Драйвер=@Драйвер, Ресурс=@Ресурс, КодШаблонаIPТелефонов=@КодШаблонаIPТелефонов
...
SELECT	КодОборудования, КодТипаОборудования, ТипОборудования, КодМоделиОборудования, МодельОборудования, КодРасположения, РасположениеPath, vwОборудованиеСписок.КодСотрудника, CASE WHEN @Язык='ru' THEN Сотрудники.ФИО ELSE Сотрудники.FIO END Сотрудник --, Изменил, Изменено 
FROM	vwОборудованиеСписок INNER JOIN Сотрудники ON vwОборудованиеСписок.КодСотрудника = Сотрудники.КодСотрудника
WHERE	vwОборудованиеСписок.КодСотрудника = @id AND (@IT=0 OR @IT = 1 AND КодТипаОборудования < 100)
	AND Списано IS NULL
...
";
        ...
        /// <summary>
        ///     Строка запроса: Вставка шаблонов конфигурации IP-телефонов
        /// </summary>
        public const string INSERT_ШаблоныIPТелефонов = @"
INSERT  ШаблоныIPТелефонов (Шаблон)
SELECT  @Шаблон
SELECT  SCOPE_IDENTITY() КодШаблонаIPТелефонов;";

        /// <summary>
        ///     Строка запроса: Обновление шаблонов конфигурации IP-телефонов
        /// </summary>
        public const string UPDATE_ШаблоныIPТелефонов = @"
UPDATE  ШаблоныIPТелефонов 
SET     Шаблон = @Шаблон
WHERE   КодШаблонаIPТелефонов = @КодШаблонаIPТелефонов";
...
SELECT  КодБизнесПроекта Id, R-L ЕстьДети, БизнесПроект Text, ISNULL(Parent,0) ParentId, '' Фильтр
FROM vwБизнесПроекты
WHERE   ((@Потомки = 1 AND ((@Код = 0 AND Parent IS NULL) OR (@Код<> 0 AND Parent = @Код)))
        OR (@Потомки = 0 AND КодБизнесПроекта = @Код)) 
ORDER BY {0}";
...
WHERE   ((@Потомки = 1 AND ((@Код = 0 AND r.Parent IS NULL) OR (@Код <> 0 AND r.Parent = @Код)))
        ...
        OR (@Потомки = 2 AND (r.КодСтатьиДвиженияДенежныхСредств IN ({1}) OR r.Parent IN ({1}))) 
ORDER BY r.{0}
";
...
FROM    (SELECT _КодДокумента КодДокумента, КодТипаДокумента, НомерДокумента, ДатаДокумента
        ...
        WHERE КодТипаДокумента=2356 
            ...
            AND ((@ЧтоОрганизовать  = 1 AND Int6 = 1 AND КодРасположения1=@КодРасположения) OR (Int6 = @ЧтоОрганизовать AND КодРасположения1 IS NULL))
            AND КодДокумента <> @КодДокумента
...
FROM vw_Д_ГТД T0 (nolock) ";
        ...
        /// <summary>
        ///     Обновление состояния сотрудникаСотрудникСостояние
        /// </summary>
