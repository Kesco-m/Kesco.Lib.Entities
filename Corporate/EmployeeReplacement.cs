...
namespace Kesco.Lib.Entities.Corporate
{
    ...
    public class EmployeeReplacement : Entity
    {
        ...
        public void LoadFromDbReader(DBReader dbReader)
        {
                ...
                int colКодЗамещенияСотрудников = dbReader.GetOrdinal("КодЗамещенияСотрудников");
                int colДо = dbReader.GetOrdinal("До");
                int colКодСотрудникаЗамещаемого = dbReader.GetOrdinal("КодСотрудникаЗамещаемого");
                int colЗамещённый = dbReader.GetOrdinal("Замещённый");
                int colКодСотрудникаЗамещающего = dbReader.GetOrdinal("КодСотрудникаЗамещающего");
                int colИспОбязанности = dbReader.GetOrdinal("ИспОбязанности");
                int colПримечания = dbReader.GetOrdinal("Примечания");
                int colИзменил = dbReader.GetOrdinal("Изменил");
                int colИзменено = dbReader.GetOrdinal("Изменено");

                Unavailable = false;

                if (!dbReader.IsDBNull(colКодЗамещенияСотрудников)) ReplacementId = dbReader.GetInt32(colКодЗамещенияСотрудников);
                if (!dbReader.IsDBNull(colДо)) ForDate = dbReader.GetDateTime(colДо);
                if (!dbReader.IsDBNull(colКодСотрудникаЗамещаемого)) EmployeeId = dbReader.GetInt32(colКодСотрудникаЗамещаемого);
                if (!dbReader.IsDBNull(colЗамещённый)) ReplacedEmployeeName = dbReader.GetString(colЗамещённый);
                if (!dbReader.IsDBNull(colКодСотрудникаЗамещающего)) VicariousId = dbReader.GetInt32(colКодСотрудникаЗамещающего);
