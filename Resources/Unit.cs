        ...
        public void FillData(string id)
        {
            ...
            using (var dbReader = new DBReader(SQLQueries.SELECT_ID_ЕдиницаИзмерения, id.ToInt(), CommandType.Text, CN))
            {
                if (dbReader.HasRows)
                {
                    ...
                    var colКодГруппыЕдиниц = dbReader.GetOrdinal("КодГруппыЕдиниц");
                    var colКоэффициент = dbReader.GetOrdinal("Коэффициент");
                    var colОписание = dbReader.GetOrdinal("Описание");
                    var colКодОКЕИ = dbReader.GetOrdinal("КодОКЕИ");
                    ...
                    if (dbReader.Read())
                    {
                        ...
                        if (!dbReader.IsDBNull(colКодГруппыЕдиниц))
                            КодГруппыЕдиниц = dbReader.GetInt32(colКодГруппыЕдиниц);
            ...
            using (var dbReader = new DBReader(query, CommandType.Text, CN))
            {
                if (dbReader.HasRows)
                {
                    ...
                    while (dbReader.Read())
                    {
                        ...
                        if (!dbReader.IsDBNull(colКодГруппыЕдиниц))
                            row.КодГруппыЕдиниц = dbReader.GetInt32(colКодГруппыЕдиниц);
