...
namespace Kesco.Lib.Entities.Documents.EF.MTR
{
    ...
    public class MtrChildDoc : Entity, ICloneable<MtrChildDoc>
    {
        ...
        {
            get { return string.IsNullOrEmpty(_documentName) ? TypeName + " №" + Number + " от " + DocDate.ToString("dd.MM.yyyy") : _documentName; }
        ...
        public static List<MtrChildDoc> GetAllLinkedDocs(int mtrDocId)
        {
            ...
            using (var dbReader = new DBReader(SQLGetAllLinkedDocs, mtrDocId, CommandType.Text, ConnString))
            {
                if(dbReader.HasRows)
                {
                   ...
                   int colКодПозицииЗаявокМТРСвязи = dbReader.GetOrdinal("КодПозицииЗаявокМТРСвязи");
                   int colКодПозицииЗаявокМТР = dbReader.GetOrdinal("КодПозицииЗаявокМТР");
                   int colТипСвязанногоДокумента = dbReader.GetOrdinal("ТипСвязанногоДокумента");
                   ...
                   while (dbReader.Read())
                   {
                       ...
                       if (!dbReader.IsDBNull(colКодДокументаОснования)) { row.ParentId = dbReader.GetInt32(colКодДокументаОснования); }
                       if (!dbReader.IsDBNull(colКодРесурса1)) { row.ResourceId = dbReader.GetInt32(colКодРесурса1); }
                       if (!dbReader.IsDBNull(colКодПозицииЗаявокМТРСвязи)) { row.PositionLinkId = dbReader.GetInt32(colКодПозицииЗаявокМТРСвязи); }
                       if (!dbReader.IsDBNull(colКодПозицииЗаявокМТР)){row.MtrPositionId = dbReader.GetInt32(colКодПозицииЗаявокМТР);}
                       if (!dbReader.IsDBNull(colТипСвязанногоДокумента)) { row.LinkType = (MtrChildType)dbReader.GetInt32(colТипСвязанногоДокумента); }
...
SELECT * FROM @ДоговораМТР";
        ...
        /// </summary>
        public const string SQLGetPayDocs = @"SELECT ДД._КодДокумента as КодДокумента, ТД.КодТипаДокумента, ТД.ТипДокумента, ДД.НомерДокумента, ДД.ДатаДокумента, ДД.Money1, Связи.КодДокументаОснования, ДД.КодРесурса1
  FROM vwСвязиДокументов Связи (nolock)
  		INNER JOIN vwДокументыДокументыДанные ДД (nolock) ON Связи.КодДокументаВытекающего = ДД._КодДокумента
  		INNER JOIN ТипыДокументов ТД (nolock) ON ДД.КодТипаДокумента = ТД.КодТипаДокумента	 
