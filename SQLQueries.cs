...
SELECT {0} T0.КодРасположения, T0.Расположение, T0.РабочееМесто, T0.Закрыто, T0.РасположениеPath0, T0.РасположениеPath1, T0.Parent, T0.L, T0.R
...
ORDER BY LEN(ТелефонныйКод) DESC";
        ...
        #region Тарификация мобильной связи
         ...
         /// </summary>
         public const string SELECT_ID_ДоговорСотовойСвязи = @"SELECT КодДоговора, Договор FROM Инвентаризация.dbo.ДоговораСотовойСвязи Д (nolock) WHERE КодДоговора = @id";
...
FROM Инвентаризация.dbo.ДоговораСотовойСвязи T0 (nolock) 
...
SELECT  T0.КодЛица, T0.ТипЛица, T0.КодБизнесПроекта, T0.Проверено, T0.Кличка, T0.КличкаRL, T0.НазваниеRL, T0.КодТерритории,
        T0.ГосОрганизация, T0.ИНН, T0.ОГРН, T0.ОКПО, T0.БИК, T0.КорСчет, T0.БИКРКЦ, T0.SWIFT, T0.ДатаРождения,T0.ДатаКонца, T0.Примечание
FROM    vwЛица T0 (nolock)
WHERE   T0.КодЛица = @id";

		 public const string SELECT_TEST_ЛицоДействует = @"
...
SELECT 0";
         ...
         /// </summary>
         public const string SELECT_ID_КарточкаФизЛица = "SELECT * FROM vwКарточкиФизЛиц WHERE КодКарточкиФизЛица = @id";

         /// <summary>
         /// Строка запроса: Получить карточку юридического лица по ID
         /// </summary>
         public const string SELECT_ID_КарточкаЮрЛица = "SELECT * FROM vwКарточкиЮрЛиц WHERE КодКарточкиЮрЛица=@id";

         /// <summary>
         /// Строка запроса: Получить карточки физического лица по ID
         /// </summary>
         public const string SELECT_ID_КарточкиФизЛица = "SELECT * FROM vwКарточкиФизЛиц WHERE КодЛица = @id ORDER BY От";
        
         /// <summary>
         /// Строка запроса: Получить карточки юридического лица по ID
         /// </summary>
         public const string SELECT_ID_КарточкиЮрЛица = "SELECT * FROM vwКарточкиЮрЛиц WHERE КодЛица = @id ORDER BY От";
...
UNION ALL
SELECT '2' + CONVERT(varchar, КодКарточкиЮрЛица) AS КодКонтакта, 1 AS КодТипаКонтакта, АдресЮридический Контакт, КодЛица, От, До FROM vwКарточкиЮрЛиц WHERE @ИсточникКонтакта&2=2
UNION ALL
SELECT '4'+ CONVERT(varchar, КодКарточкиФизЛица) AS КодКонтакта, 1 AS КодТипаКонтакта, АдресЮридический Контакт, КодЛица, От, До FROM vwКарточкиФизЛиц WHERE @ИсточникКонтакта&4=4) AS T0
WHERE
Контакт<>''";
...
UNION ALL
SELECT КодКарточкиЮрЛица AS КодКонтакта, '2' AS КодТипаКонтакта, АдресЮридический Контакт, КодЛица, От, До FROM vwКарточкиЮрЛиц WHERE 2=CAST(LEFT(@КодКонтакта,1) as int)&2
UNION ALL
SELECT КодКарточкиФизЛица AS КодКонтакта, '4' AS КодТипаКонтакта, АдресЮридический Контакт, КодЛица, От, До FROM vwКарточкиФизЛиц WHERE 4=CAST(LEFT(@КодКонтакта,1) as int)&4) AS T0
WHERE
...
UNION ALL
SELECT КодКарточкиЮрЛица AS КодКонтакта, КодЛица FROM vwКарточкиЮрЛиц WHERE 2=CAST(LEFT(@КодКонтакта,1)&2 as int)&2
UNION ALL
SELECT КодКарточкиФизЛица AS КодКонтакта, КодЛица FROM vwКарточкиФизЛиц WHERE 4=CAST(LEFT(@КодКонтакта,1) as int)&4) AS T0
WHERE
КодКонтакта= CAST( SUBSTRING (@КодКонтакта, 2, 30) as int)
AND КодЛица= @КодЛица)
SELECT 1
...
UNION ALL
SELECT КодКарточкиЮрЛица AS КодКонтакта, От, До FROM vwКарточкиЮрЛиц WHERE 2=CAST(LEFT(@КодКонтакта,1)&2 as int)&2
UNION ALL
SELECT КодКарточкиФизЛица AS КодКонтакта, От, До FROM vwКарточкиФизЛиц WHERE 4=CAST(LEFT(@КодКонтакта,1) as int)&4) AS T0
WHERE
КодКонтакта= CAST( SUBSTRING (@КодКонтакта, 2, 30) as int)
AND
...
SELECT {0}  T0.КодРесурса, ISNULL(NULLIF(T0.РесурсРус,''),'#' + CONVERT(varchar,T0.КодРесурса)) РесурсРус, T0.РесурсЛат, T0.РесурсRL,
            T0.КодЕдиницыИзмерения, T0.КодВидаПодакцизногоТовара, T0.Точность, T0.СпецНДС, T0.Parent, T0.L, T0.R
...
FROM    Ресурсы parent (nolock) INNER JOIN 
        ...
        /// <summary>
        /// Строка запроса: Получение склада по его идентификатору
        /// </summary>
        //public const string SELECT_Склады = @"SELECT КодСклада, Склад, IBAN, От, DATEADD(day,-1,До) AS До, Действует, КодТипаСклада, КодМестаХранения, КодРесурса, КодХранителя, КодРаспорядителя, КодПодразделенияРаспорядителя, КодДоговора, Филиал, Примечание, Изменил, Изменено FROM vwСклады WHERE КодСклада=@id";
...
SELECT {0} КодСклада, Склад, COALESCE( NULLIF(Склад, '') + '/' + NULLIF(IBAN, ''), NULLIF(Склад, ''), NULLIF(IBAN, ''), '_____________________') AS Название
FROM    vwСклады T0 WHERE КодСклада=@КодСклада";
...
FROM vwДокументы T0 (nolock) ";
        ...
        /// <summary>
        ///  Получить запрос: Получить все вытекающие документы
        /// </summary>
...
ORDER BY    КодТарификацииСотовых DESC";
        ...
        public const string SQL_ДвиженияНаСкладах =
            @"SELECT
                vwДвиженияНаСкладах.КодДвиженияНаСкладе,   
                isnull(vwОтправкаВагонов.ОтправкаВагона,'') ОтправкаВагона,
                vwДвиженияНаСкладах.РесурсРус, 
                vwДвиженияНаСкладах.Количество, 
                ЕдиницыИзмерения.ЕдиницаРус, 
                vwДвиженияНаСкладах.ЦенаБезНДС,
                СтавкиНДС.Величина * 100 as Величина100,
                vwДвиженияНаСкладах.СуммаБезНДС, 
                vwДвиженияНаСкладах.СуммаНДС, 
                vwДвиженияНаСкладах.Всего, 
                isnull(Территории.Территория,'') as Территория, 
                isnull(vwДокументы.НазваниеДокумента,'') as НазваниеДокумента
            FROM vwДвиженияНаСкладах (nolock)
            LEFT JOIN Справочники.dbo.ЕдиницыИзмерения ON ЕдиницыИзмерения.КодЕдиницыИзмерения = vwДвиженияНаСкладах.КодЕдиницыИзмерения
            LEFT JOIN Справочники.dbo.СтавкиНДС ON СтавкиНДС.КодСтавкиНДС = vwДвиженияНаСкладах.КодСтавкиНДС
            LEFT JOIN Инвентаризация.dbo.Территории ON Территории.КодТерритории = vwДвиженияНаСкладах.КодСтраныПроисхождения
            LEFT JOIN vwДокументы ON vwДокументы.КодДокумента = vwДвиженияНаСкладах.КодТаможеннойДекларации
            LEFT JOIN vwОтправкаВагонов ON vwОтправкаВагонов.КодОтправкиВагона = vwДвиженияНаСкладах.КодОтправкиВагона
            WHERE vwДвиженияНаСкладах.КодДокумента = @КодДокумента
            ORDER BY vwДвиженияНаСкладах.Порядок
            ";

        public const string SQL_ДвиженияНаСкладахПоРесурсу =
            @"SELECT
	            res.РесурсРус		РесурсРус
	            ,SUM(d.Количество)	Количество
	            ,ei.ЕдиницаРус		ЕдиницаРус
	            ,SUM(d.ЦенаБезНДС)	ЦенаБезНДС
	            ,СтавкиНДС.Величина * 100 as Величина100
	            ,SUM(d.СуммаНДС)	СуммаНДС
	            ,SUM(d.СуммаБезНДС)	СуммаБезНДС
	            ,SUM(d.Всего)		Всего
            FROM vwДвиженияНаСкладах d (nolock)
	            INNER JOIN Справочники.dbo.Ресурсы res (nolock) on res.КодРесурса=d.КодРесурса
	            LEFT JOIN Справочники.dbo.СтавкиНДС on СтавкиНДС.КодСтавкиНДС=d.КодСтавкиНДС
	            LEFT JOIN Справочники.dbo.ЕдиницыИзмерения ei on ei.КодЕдиницыИзмерения=d.КодЕдиницыИзмерения
            WHERE КодДокумента=@КодДокумента
            GROUP BY res.РесурсРус,ei.ЕдиницаРус,СтавкиНДС.Величина * 100,res.Точность
            ";
        #endregion
        ...
        public const string SQL_ОказанныеУслуги =
            @"SELECT
                vwОказанныеУслуги.КодОказаннойУслуги,
                vwОказанныеУслуги.GuidОказаннойУслуги,
                vwОказанныеУслуги.РесурсРус, 
                vwОказанныеУслуги.Агент1,
                vwОказанныеУслуги.Агент2,
                vwОказанныеУслуги.Количество,
                ЕдиницыИзмерения.ЕдиницаРус, 
                vwОказанныеУслуги.ЦенаБезНДС,
                СтавкиНДС.Величина * 100 as Величина100,
                vwОказанныеУслуги.СуммаБезНДС,
                vwОказанныеУслуги.СуммаНДС,
                vwОказанныеУслуги.Всего
            FROM vwОказанныеУслуги
            LEFT JOIN Справочники.dbo.ЕдиницыИзмерения ON ЕдиницыИзмерения.КодЕдиницыИзмерения = vwОказанныеУслуги.КодЕдиницыИзмерения
            LEFT JOIN Справочники.dbo.СтавкиНДС ON СтавкиНДС.КодСтавкиНДС = vwОказанныеУслуги.КодСтавкиНДС
            WHERE vwОказанныеУслуги.КодДокумента = @КодДокумента
            ";
        #endregion
