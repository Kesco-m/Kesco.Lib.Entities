...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_ДопПараметрыУказанийИТ]') AND type in (N'P', N'PC'))
    EXEC sp_ДопПараметрыУказанийИТ @КодСотрудника
ELSE
    SELECT 0 КодДопПараметраУказанийИТ WHERE 1 = 0";
...
FROM vw_Д_ГТД T0 (nolock) ";
        ...
        public const string SELECT_ИдентификаторОценкиИнтерфейса = @"
            SELECT COUNT(КодИдентификатораОценки) AS ИдентификаторОценки FROM ИдентификаторыОценок WHERE КодИдентификатораОценки = @КодИдентификатораОценки AND Отключено = 0";

        /// <summary>
        ///     Получение состояния Нравится - не нравится
        /// </summary>
        public const string SELECT_ОценкиИнтерфейса = @"
            SELECT Оценка = [dbo].[fn_ИтоговаяОценкаСотрудника] (@КодИдентификатораОценки, @КодСотрудника)";
