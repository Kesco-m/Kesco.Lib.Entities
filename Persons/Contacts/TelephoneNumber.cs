...
namespace Kesco.Lib.Entities.Contacts
{
    ...
    public class TelephoneNumber : Entity
    {
        ...
        {
            var sqlParams = new Dictionary<string, object>
                {{"@phone", new object[] {PhoneNumber, DBManager.ParameterTypes.String}}};
        ...
        {
            var sqlParams = new Dictionary<string, object>
                {{"@id", new object[] {TerritoryID, DBManager.ParameterTypes.Int32}}};
            FillData(DBManager.GetData(SQLQueries.SELECT_ID_Территории_ТелефонныеКоды, CN, CommandType.Text,
                sqlParams));
        }
        ...
        protected override void FillData(DataTable dt)
        {
            if (dt.Rows.Count == 1)
            {
                var countyTel = dt.Rows[0]["ТелКодСтраны"].ToString();
                var cityTel = dt.Rows[0]["ТелКодВСтране"].ToString();
