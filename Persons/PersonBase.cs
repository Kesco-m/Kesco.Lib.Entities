...
using Kesco.Lib.Entities.Persons.Dossier;
using Kesco.Lib.Web;
using System.Linq;
...
namespace Kesco.Lib.Entities.Persons
{
    ...
    public class PersonBase : Entity
    {
        ...
        public object AddPersonEmployes(int? empoyeePersonID, int personID)
        {
            ...
            {
                Lib.Log.Logger.WriteEx(new Lib.Log.DetailedException("Ошибка при создании ответвенных", ex));
                throw ex;
            }
        }
        ...
        {
            string sql = String.Format(@"SELECT Инвентаризация.dbo.fn_TransLit('{0}')", rusValue);
            DataTable resultTable = DBManager.GetData(sql, Config.DS_person);
