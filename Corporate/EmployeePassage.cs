...
namespace Kesco.Lib.Entities.Corporate
{
    ...
    public class EmployeePassage : Entity
    {
        ...
        public override void Load()
        {
            ...
            using (var dbReader = new DBReader(SQLQueries.SELECT_ПоследнийПроходСотрудника, CommandType.Text, CN,
                sqlParams))
            {
                if (dbReader.HasRows)
                {
                    ...
                    var colPoint = dbReader.GetOrdinal("Считыватель");
                    var colTimeAt = dbReader.GetOrdinal("ПоследнийПроход");
